...
const PopularAlbum = () => {
    ...
    // Function to fetch latest albums from Spotify API
    const fetchLatestAlbums = async (token) => {
      ...
      const data = await response.json();
      setLatestAlbums(data.albums.items);
      setLoading(false)
    };
  ...
  return (
    <div className='w-full'>
      <div className='flex justify-between items-center'>
        <h1 className='font-semibold text-3xl text-white mb-7'>Popular Song</h1>
      </div>
      {loading? (
      ...
      <div key={index} className='w-56 min-h-64 bg-neutral-800  flex justify-center rounded-lg cursor-pointer relative animate-pulse '>
        ...
        <div className='flex overflow-x-auto snap-x gap-4 overflow-y-hidden transition duration-300 ease-in-out' ref={scrollElement}>
            ...
            {latestAlbums.map((track) => (
             <Link to={`/album/${track.id}`} key={track.id}>
              <div key={track.id} className='min-w-56 min-h-64 hover:bg-neutral-800 flex justify-center rounded-lg cursor-pointer relative'>
                <div className='w-52 min-h-64'>
                  <div className='flex justify-center mt-3'>
